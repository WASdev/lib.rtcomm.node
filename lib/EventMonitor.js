...
function EventMonitor(config) {
  ...
  var createMqttClient = function(config) {
    ...
    client.on('connect', function(error) {
      console.log('EventMonitor: mqtt is CONNECTED');
      this.connected = true;
      this.emit('connected');
    }.bind(this));
    client.on('error', function(error) {
      console.log('EventMonitor: DISCONNECTED - error:'+ error);
      this.connected = false;
      this.emit('disconnected', error);
    }.bind(this));
    client.on('close', function(error) {
      console.log('EventMonitor: DISCONNECTED - close: '+error);
      this.connected = false;
      this.emit('disconnected',error);
    }.bind(this));
    return client;
 ...
 */
  ...
  this.destroy = function destroy() {
	console.log("EventMonitor: destroy top");
	...
	if (eventMonitors[this.id]){
		console.log("EventMonitor: destroy: delete this eventMonitor: "+ this.id);
		delete eventMonitors[this.id];
	}
  };
